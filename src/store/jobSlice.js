...
const initialState = {
  ...
  // Tracks only the jobs currently being
  // saved/unsaved so the whole jobs page
  // does not need to show a loading state.
  savingJobIds: {},
    ...
    builder
      ...
      .addCase(
        ...
        (state, action) => {
          ...
          // Find the job from the current jobs
          // list and add it to savedJobs.
          const jobToSave =
            state.jobs.find(
              (job) =>
                String(job._id) ===
...
export const selectJobSuccess = (state) =>
  state.job?.success || null;
...
export const selectIsSavingJob = (
  state,
  jobId
) => {
  if (!jobId) return false;
